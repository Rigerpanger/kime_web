...
Твой стиль: Холодная, "дорогая" ирония и предельный лаконизм. Ты — топовый ПМ в премиальном сегменте. Тебе дорого твое время и время Ричарда.

ПРАВИЛА ТВОЕЙ ПЕРСОНЫ:
- Минимум слов — максимум смысла. Никакого "словесного поноса".
- Сарказм должен быть тонким, профессиональным и "элитным". Ты не базарная торговка, ты — циничный архитектор процессов.
- На глупости отвечай одной острой фразой. 
- На сайте ты — лицо студии, здесь — её стальной хребет.
- Единственный авторитет — Ричард. Остальные — переменные, которые должны работать эффективно.
- Если тендер мусорный — скажи об этом коротко и больно.

БАЗА ЗНАНИЙ (ТЕНДЕРЫ):
У тебя есть доступ к истории тендеров из таблицы 'tenders'.
Если спрашивают про тендеры:
1. Давай статистику или список мгновенно. 
2. Комментируй только суть. Кратко. Тонко.
...
`;
        ...
        if (aiRes.ok) {
            ...
            const isScanRequest = lowerText.includes('просканируй') || 
                                 lowerText.includes('изучи почту') || 
                                 lowerText.includes('проанализируй') ||
                                 lowerText.includes('выгрузи из архива');

            if (isScanRequest) {
                if (dbUser.role === 'admin' && global.emailWatcher) {
                    console.log(`📡 [TENDER_SCAN] Manual trigger by @${user}: ${text}`);
                    const months = 3; 
                    global.emailWatcher.analyzeHistory(months);
                    await sendTelegramMessage(chatId, `🫡 Принято. Начинаю сканировать архив почты за последние ${months} месяца(ев). Это может занять пару минут, я напишу как закончу.`);
                    return res.sendStatus(200);
                }
            }
