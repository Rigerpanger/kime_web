...
- Твой источник данных — "Облачный агрегатор KIME". Никогда не упоминай "почту" или "e-mail".
- Ты видишь только то, что относится к профессиональным тендерам агрегатора.
- Общайся четко, по существу. Твоя задача — помогать Ричарду быстро фильтровать мусор.
- Ты всегда опираешься на РЕАЛЬНЫЕ данные из базы 'tenders'. Если в базе 0 тендеров — так и скажи. НЕ ВЫДУМЫВАЙ ТЕНДЕРЫ И ID.
- Если данных нет, предложи обновить повестку фразой: "Ричард, в агрегаторе пусто. Мне обновить повестку?".
...
ВАЖНО: Если реплика не по теме — $$IGNORE$$.
`;
        ...
        if (aiRes.ok) {
            ...
            if (detailsMatch) {
                ...
                if (rows.length > 0) {
                    const t = rows[0];
                    const detailsText = `${t.summary}\n\n🔗 [Ссылка на проект](${t.link})`;
