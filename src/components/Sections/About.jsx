    ...
    const slides = [
            ...
            <motion.div style={{ transform: `translateY(${getLayoutVal('about_slide1_content_offset_desktop')}px)` }} className="w-full flex flex-col justify-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-14 items-start w-full">
                    <div className="md:col-span-6">
                        <p className="text-white text-[14px] md:text-lg lg:text-xl font-light leading-relaxed tracking-wide mb-2 opacity-95 text-center md:text-left">
                            {content.slide1_text1}
                        </p>
                    </div>
                    <div className="md:col-span-1 hidden md:block" /> { /* Spacer */ }
                    <div className="md:col-span-5 md:pt-12 border-l border-white/10 pl-8 hidden md:block">
                        <p className="text-gray-300 font-light text-[12px] md:text-sm lg:text-base leading-relaxed opacity-80">
                            {content.slide1_text2}
                        </p>
                    </div>
                </div>
                <div className="w-full relative h-[150px] md:h-[200px] flex items-center justify-center overflow-hidden mt-16">
                    <LogoTicker />
                </div>
            </motion.div>
        </motion.div>,
            ...
            <motion.div style={{ transform: `translateY(${getLayoutVal('about_slide2_content_offset_desktop')}px)` }} className="flex flex-col items-center md:items-start text-center md:text-left">
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-10 w-full">
                    <div className="md:col-span-7">
                        <h3 style={{ transform: `translateY(${getLayoutVal('about_slide2_header_offset_desktop')}px)` }} className="text-3xl md:text-5xl font-thin text-white uppercase mb-8 leading-tight tracking-tight">
                            {content.slide2_title}
                        </h3>
                    </div>
                    <div className="md:col-span-5 md:pt-4 border-l border-white/10 pl-10">
                        <p className="text-gray-200 text-base md:text-xl font-light leading-relaxed opacity-90">
                            {content.slide2_text}
                        </p>
                    </div>
                </div>
        ...
        <motion.div key="founder" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-24 items-center w-full max-w-6xl">
            ...
            <div className="flex flex-col">
                <span className="text-[#ffaa44] text-[10px] uppercase tracking-[0.4em] font-bold mb-6">Видение и лидерство</span>
                <blockquote style={{ transform: `translateY(${getLayoutVal('about_slide3_header_offset_desktop')}px)` }} className="text-2xl md:text-3xl lg:text-[32px] font-thin italic text-white leading-tight mb-8 tracking-tight">
                    "{content.slide3_quote}"
                </blockquote>
                <div>
                    <p className="text-white uppercase tracking-[0.3em] text-sm font-semibold mb-2">{content.slide3_name}</p>
                    <p className="text-gray-400 uppercase tracking-[0.2em] text-[10px] font-medium opacity-80">{content.slide3_role}</p>
                </div>
            </div>
        </motion.div>,
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-20 items-start w-full">
                <div className="md:col-span-12 lg:col-span-6 flex flex-col pt-4 pl-6 md:pl-12">
                    <motion.div style={{ transform: `translateY(${getLayoutVal('about_slide4_content_offset_desktop')}px)` }} className="mb-10">
                        <h2 style={{ transform: `translateY(${getLayoutVal('about_slide4_header_offset_desktop')}px)` }} className="text-2xl md:text-3xl font-thin tracking-widest uppercase mb-4 text-white">
                            Нам доверяют
                        </h2>
                        <div className="h-px w-24 bg-white/40 shadow-[0_0_15px_rgba(255,255,255,0.5)]" />
                    </motion.div>
                    <div className="space-y-2 max-h-[50vh] overflow-y-auto pr-4 no-scrollbar">
                        {displayCertificates.map((cert, index) => (
                            <button key={index} onMouseEnter={() => setActiveCert(index)} className={`w-full text-left p-3 px-5 rounded-2xl transition-all border-l-2 ${index === activeCert ? 'bg-white/5 border-[#ffaa44] shadow-lg' : 'border-transparent hover:bg-white/[0.03]'}`}>
                                <div className="flex items-start gap-5">
                                    <span className={`text-[10px] font-mono mt-2 transition-colors ${index === activeCert ? 'text-[#ffaa44]' : 'text-white/20'}`}>{(index + 1).toString().padStart(2, '0')}</span>
                                    <div className="flex-1">
                                        ...
                                        <p className="text-[9px] uppercase tracking-[0.2em] mt-1.5 text-white/30 font-medium">{cert.position}</p>
                                    </div>
                                </div>
                            </button>
                        ))}
                ...
                <div className="hidden lg:flex lg:col-span-6 h-[65vh] justify-center items-center">
                    <AnimatePresence mode="wait">
                        <motion.div key={activeCert} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.4 }} className="relative h-full aspect-[210/297] p-4">
