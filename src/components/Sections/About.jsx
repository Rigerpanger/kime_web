...
const About = () => {
    ...
    const slides = [
        <motion.div key="studio" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col w-full relative">
            <motion.div style={{ transform: `translateY(${getLayoutVal('about_slide1_content_offset_desktop') - 5}px)` }} className="w-full flex flex-col justify-center">
                <h2 style={{ transform: `translateY(${getLayoutVal('about_slide1_header_offset_desktop')}px)`, textShadow: '0 4px 20px rgba(0,0,0,0.6)' }} className="text-3xl md:text-4xl font-thin mb-4 text-white uppercase tracking-widest text-center md:text-left">
                    {content.slide1_title}
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-14 items-start w-full">
                    <div className="md:col-span-6">
                        <p className="text-white/90 text-[12px] md:text-sm lg:text-base font-light leading-relaxed tracking-wide mb-2 text-center md:text-left">
                            {content.slide1_text1}
                        </p>
                    </div>
                    <div className="md:col-span-1 hidden md:block" /> { /* Spacer */ }
                    <div className="md:col-span-5 md:pt-4 border-l border-white/5 pl-8 hidden md:block">
                        <p className="text-gray-400/80 font-light text-[10px] md:text-[11px] lg:text-xs leading-relaxed">
                            {content.slide1_text2}
                        </p>
                    </div>
                </div>
                <div className="w-full relative h-[70px] flex items-center justify-center overflow-hidden mt-2">
                    <LogoTicker />
                </div>
            </motion.div>
        </motion.div>,
        ...
        <motion.div key="founder" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20 items-center w-full">
            <motion.div style={{ transform: `translateY(${getLayoutVal('about_slide3_content_offset_desktop')}px)` }} className="h-[30vh] md:h-[45vh] flex justify-end">
                <div className="w-[80%] h-full bg-zinc-900 rounded-2xl overflow-hidden grayscale border border-white/5 relative shadow-2xl">
            ...
            <div className="flex flex-col">
                <span className="text-[#ffaa44]/80 text-[8px] uppercase tracking-[0.4em] font-bold mb-4">Видение и лидерство</span>
                <blockquote style={{ transform: `translateY(${getLayoutVal('about_slide3_header_offset_desktop')}px)` }} className="text-base md:text-xl italic text-white/90 leading-snug mb-6 tracking-tight">
                    "{content.slide3_quote}"
                </blockquote>
                <div>
                    <p className="text-white/80 uppercase tracking-[0.3em] text-[10px] font-semibold mb-1">{content.slide3_name}</p>
                    <p className="text-gray-400/60 uppercase tracking-[0.2em] text-[8px] font-medium">{content.slide3_role}</p>
                </div>
            </div>
        </motion.div>,
        <motion.div key="certificates" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center w-full">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-20 items-start w-full">
                <div className="md:col-span-12 lg:col-span-6 flex flex-col pt-2 pl-6 md:pl-12">
                    <motion.div style={{ transform: `translateY(${getLayoutVal('about_slide4_content_offset_desktop')}px)` }} className="mb-6">
                        <h2 style={{ transform: `translateY(${getLayoutVal('about_slide4_header_offset_desktop')}px)` }} className="text-lg md:text-xl font-thin tracking-[0.3em] uppercase mb-4 text-white/90">
                            Нам доверяют
                        </h2>
                        <div className="h-[1px] w-12 bg-white/10" />
                    </motion.div>
                    <div className="space-y-0.5 max-h-[35vh] overflow-y-auto pr-6 no-scrollbar">
                        {displayCertificates.map((cert, index) => (
                            <button key={index} onMouseEnter={() => setActiveCert(index)} className={`w-full text-left p-2 px-6 rounded-2xl transition-all border-l-2 ${index === activeCert ? 'bg-white/5 border-[#ffaa44]/70 shadow-lg' : 'border-transparent hover:bg-white/[0.02]'}`}>
                                <div className="flex items-start gap-6">
                                    ...
                                    <div className="flex-1">
                                        <p className={`text-sm md:text-base font-light tracking-wide ${index === activeCert ? 'text-white font-normal' : 'text-white/50'}`}>{cert.company} {cert.division}</p>
                                        <p className="text-[8px] uppercase tracking-[0.2em] mt-0.5 text-white/20 font-medium">{cert.position}</p>
                                    </div>
                                </div>
                            </button>
                        ))}
                ...
                <div className="hidden lg:flex lg:col-span-6 h-[50vh] justify-center items-center">
                    <AnimatePresence mode="wait">
                        <motion.div key={activeCert} initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} transition={{ duration: 0.4 }} className="relative h-full aspect-[210/297] p-6">
                            <div className="absolute inset-0 bg-white/[0.03] backdrop-blur-3xl rounded-3xl border border-white/5 shadow-2xl" />
                            <div className="h-full w-full relative z-10 rounded-2xl overflow-hidden shadow-2xl grayscale cursor-zoom-in" onClick={() => setSelectedFullCert(displayCertificates[activeCert])}>
                                <img src={displayCertificates[activeCert]?.image_url} alt="Certificate" className="w-full h-full object-cover opacity-80 transition-opacity hover:opacity-100" />
                            </div>
                        </motion.div>
                    </AnimatePresence>
                </div>
    ...
    return (
        <section id="about" className="w-full h-screen relative overflow-hidden pointer-events-none">
            ...
            {!isMobile && (
                <>
                    <button onClick={prevSlide} className="fixed left-8 top-1/2 -translate-y-1/2 z-[40] p-6 text-white/10 hover:text-white transition-all pointer-events-auto hover:scale-110"><ChevronLeft size={54} strokeWidth={0.2} /></button>
                    <button onClick={nextSlide} className="fixed right-8 top-1/2 -translate-y-1/2 z-[40] p-6 text-white/10 hover:text-white transition-all pointer-events-auto hover:scale-110"><ChevronRight size={54} strokeWidth={0.2} /></button>
                </>
            )}
