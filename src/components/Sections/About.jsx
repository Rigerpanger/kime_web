...
const About = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (contentRes.ok) setContent(prev => ({ ...prev, ...contentRes.json() }));
    ...
    const slides = [
        <motion.div key="studio" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col max-w-4xl w-full relative">
            <motion.div style={{ transform: `translateY(${getLayoutVal('about_slide1_content_offset_desktop')}px)` }} className="w-full flex flex-col justify-center">
                <h2 style={{ transform: `translateY(${getLayoutVal('about_slide1_header_offset_desktop')}px)`, textShadow: '0 4px 20px rgba(0,0,0,0.6)' }} className="text-3xl md:text-5xl lg:text-5xl font-thin mb-8 text-white uppercase tracking-wider text-center md:text-left">
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-14 items-center w-full">
                    ...
                    <div className="md:col-span-5 border-l border-white/10 pl-6 hidden md:block">
        ...
        <motion.div key="approach" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col max-w-4xl w-full relative">
            <motion.div style={{ transform: `translateY(${getLayoutVal('about_slide2_content_offset_desktop')}px)` }} className="flex flex-col items-center md:items-start text-center md:text-left">
                ...
                <h3 style={{ transform: `translateY(${getLayoutVal('about_slide2_header_offset_desktop')}px)` }} className="text-2xl md:text-4xl font-thin text-white uppercase mb-6 leading-relaxed max-w-3xl">
                    {content.slide2_title}
                </h3>
                <p className="text-gray-100 text-base md:text-lg font-light leading-relaxed max-w-2xl opacity-90">
                    {content.slide2_text}
                </p>
            </motion.div>
        </motion.div>,
        <motion.div key="founder" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-center w-full max-w-5xl">
        ...
        <motion.div key="certificates" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center w-full max-w-6xl">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-16 items-start">
                <div className="md:col-span-12 lg:col-span-6 flex flex-col pt-4 pl-6 md:pl-16">
                    <motion.div style={{ transform: `translateY(${getLayoutVal('about_slide4_content_offset_desktop')}px)` }} className="mb-6">
                        <h2 style={{ transform: `translateY(${getLayoutVal('about_slide4_header_offset_desktop')}px)` }} className="text-xl md:text-2xl font-thin tracking-widest uppercase mb-3 text-white">
                            Нам доверяют
                        </h2>
                ...
                <div className="hidden lg:flex lg:col-span-6 h-[58vh] justify-center items-center">
